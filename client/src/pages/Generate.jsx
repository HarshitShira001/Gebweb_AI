...
function Generate() {
  ...
  const [error, setError] = useState("")   // ← moved to top level (Rules of Hooks)
  ...
  const handleGenerateWebsite = async () => {
    ...
    try {
      ...
      setLoading(false)   // ← was missing — spinner never stopped on error
    }
  }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const increment = value < 20
        ? Math.random() * 1.5    // ← Math.random() fixed (was Math.random without ())
        : value < 60
        ? Math.random() * 1.2
        : Math.random() * 0.6;
      value += increment
  ...
  return (
    ...
    >
        <h1 className='text-4xl md:text-5xl font-bold mb-5 leading-tight'>Build websites with
            <span className='block bg-gradient-to-r from-white to-zinc-400 bg-clip-text text-transparent'>Real Ai power</span> </h1>
        ...
        <div className='mb-14'>
            ...
            <div className='relative'>
                ...
                value={prompt}
                placeholder='Describe your website in detail...' className='w-full h-56 p-6 rounded-3xl bg-black/60 border border-white/10 outline-none resize-none text-sm leading-relaxed focus:ring-2 focus:ring-white/20'></textarea>
            </div>
            ...
            <div className='flex justify-center'>
                <motion.button
                    ...
                    disabled={!prompt.trim() || loading}  // ← fixed: was && (inverted logic)
            ...
            {loading && (
              ...
              >
                ...
                <div className='h-2 w-full bg-white/10 rounded-full overflow-hidden'>
                  <motion.div className="h-full bg-gradient-to-r from-white to-zinc-300"
